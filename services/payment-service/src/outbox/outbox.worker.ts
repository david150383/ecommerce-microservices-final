...
import { OutboxEventRow } from "../modules/payment/types/payment.types.js";
...
export class OutboxWorker {
  ...
  public start(): void {
    ...
    this.isRunning = true;
    logger.info("Payment outbox worker started");
    this.scheduleNext();
  }
  ...
  public stop(): void {
    ...
    logger.info("Payment outbox worker stopped");
  }
  ...
  private scheduleNext(): void {
    ...
    this.timer = setTimeout(async () => {
      try {
        ...
        logger.error("Error processing outbox events in payment service", error);
      } finally {
        this.scheduleNext();
      }
    }, this.pollIntervalMs);
    ...
    try {
      ...
      for (const event of res.rows) {
        try {
          ...
          logger.error(
            `Failed to publish payment outbox event ${event.event_id}. Retry ${nextRetry}/${this.maxRetries}`,
            publishErr,
          );
      ...
      await client.query("COMMIT");
      if (processedCount > 0) {
        logger.info(`Payment outbox worker published ${processedCount} event(s)`);
      }

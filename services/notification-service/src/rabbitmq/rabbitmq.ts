...
export async function getRabbitConnection(): Promise<amqplib.ChannelModel> {
  ...
  try {
    ...
    connection.on("error", (err) => {
      logger.error("RabbitMQ connection error in notification-service", err);
    });

    connection.on("close", () => {
      logger.warn("RabbitMQ connection closed in notification-service");
      connection = null;
      publishChannel = null;
    });
...
export async function publishEvent(
  ...
  try {
    ...
    logger.info(`Event published from notification-service: ${routingKey}`, {
...
export async function checkRabbitHealth(): Promise<boolean> {
  try {
    ...
    logger.error("RabbitMQ health check failed in notification-service", error);
    return false;
  }
}
...
export async function closeRabbitmq(): Promise<void> {
  try {
    ...
    logger.error("Error closing RabbitMQ connection in notification-service", error);
  }
}
